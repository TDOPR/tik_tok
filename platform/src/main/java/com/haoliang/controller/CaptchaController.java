...
public class CaptchaController {
    ...
    @Value("${mail.brevoApiEnabled:false}")
    private boolean brevoApiEnabled;
    ...
    @RepeatSubmit(interval = 60000, delete = false)
    ...
    public JsonResult<UuidVO> sendCaptchaToEmail(@PathVariable String email, @PathVariable Integer type) {
        Integer count = 0;
        String limitKey = CacheKeyPrefixConstants.SEND_EMAIL_LIMIT + email;
        if (RedisUtil.hasKey(limitKey)) {
            count = RedisUtil.getCacheObject(limitKey);
            //判断邮箱发送次数是否达到上限
            if (count >= TiktokConfig.SEND_EMAIL_LIMIT_SIZE) {
                return JsonResult.failureResult(ReturnMessageEnum.SEND_EMAIL_LIMIT_ERROR);
            }
        }
        ...
        String cacheKey = CacheKeyPrefixConstants.CAPTCHA_CODE + email;
        if (GlobalProperties.isProdEnv()) {
            ...
            boolean flag;
            if (brevoApiEnabled) {
                flag = EmailServer.sendBrevoEmail(emailTemplateDTO.getTitle(), emailTemplateDTO.getContent().replace("{{code}}", code).replace("{{time}}", loginConfig.getCaptcha().getExpirationTime().toString()), email);
            } else {
                flag = EmailServer.send(emailTemplateDTO.getTitle(), emailTemplateDTO.getContent().replace("{{code}}", code).replace("{{time}}", loginConfig.getCaptcha().getExpirationTime().toString()), email);
            }
            if (flag) {
                count++;
                RedisUtil.setCacheObjectTodayExpireAsync(limitKey, count);
                RedisUtil.setCacheObject(cacheKey, code, Duration.ofMinutes(loginConfig.getCaptcha().getExpirationTime()));
                return JsonResult.successResult("ok", new UuidVO("0"));
            }
