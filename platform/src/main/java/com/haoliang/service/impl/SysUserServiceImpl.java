...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public JsonResult getGoogleQRCode(LoginDTO loginDTO) {
        ...
        SysUser sysUser = this.getOne(
                new LambdaQueryWrapper<SysUser>()
                        ...
                        .eq(SysUser::getDeleted, BooleanEnum.FALSE.intValue()));
        //登录状态标识
        ...
        if (sysUser == null) {
            ...
            loginFlag = false;
        }else if (BooleanEnum.FALSE.intValue().equals(sysUser.getEnabled())) {
            return JsonResult.failureResult(ReturnMessageEnum.ACCOUNT_DISABLED);
        }
        //验证错误次数,当在指定时间内达到指定次数则锁定用户
        setFailCountAndLock(sysUser.getUsername(), loginFlag);
        if (!loginFlag) {
            //错误标识
    ...
    public JsonResult login(LoginDTO loginDTO, String clientIp) {
        ...
        //验证错误次数,当在指定时间内达到指定次数则锁定用户
        boolean lockFlag = setFailCountAndLock(sysUser.getUsername(), loginFlag);

        //异常锁定标识
        if (lockFlag) {
            UpdateWrapper<SysUser> updateWrapper = Wrappers.update();
            updateWrapper.lambda()
                    .set(SysUser::getEnabled, BooleanEnum.FALSE.intValue())
                    .eq(SysUser::getId, sysUser.getId());
            this.update(updateWrapper);
            log.error(" 用户名:{} ,后台登录密码次数错误超过:{},修改账号状态为锁定状态!", sysUser.getUsername(), loginConfig.getMaxErrorNumber());
        }
        if (!loginFlag) {
            //错误标识
            return JsonResult.failureResult(ReturnMessageEnum.PASSWORD_ERROR);
...
//    }
     ...
     * 密码锁定校验
     */
    private boolean setFailCountAndLock(String username, boolean loginFlag) {
        if (loginConfig.isEnableErrorLock()) {
            String cacheKey = CacheKeyPrefixConstants.PWD_ERROR_COUNT + username;
            if (loginFlag) {
                // 如果登录成功,清除之前的缓存
                ...
                RedisUtil.setCacheObject(cacheKey, errorNumber, Duration.ofMinutes(loginConfig.getLockTime()));
                if (errorNumber.equals(loginConfig.getMaxErrorNumber())) {
                    return true;
                }
            }
        }
    ...
    public JsonResult proxylogin(LoginDTO loginDTO, String localIp) {
        ...
        }else if (BooleanEnum.FALSE.intValue().equals(appUsers.getEnabled())) {
            return JsonResult.failureResult(ReturnMessageEnum.ACCOUNT_DISABLED);
        ...
        //验证错误次数,当在指定时间内达到指定次数则锁定用户
        boolean lockFlag = setFailCountAndLock(appUsers.getEmail(), loginFlag);

        //异常锁定标识
        if (lockFlag) {
            UpdateWrapper<AppUsers> updateWrapper = Wrappers.update();
            updateWrapper.lambda()
                    .set(AppUsers::getEnabled, BooleanEnum.FALSE.intValue())
                    .eq(AppUsers::getId, appUsers.getId());
            appUserMapper.update(null, updateWrapper);
            log.error("用户名:{} ,代理商登录密码次数错误超过:{},修改账号状态为锁定状态!", appUsers.getEmail(), loginConfig.getMaxErrorNumber());
        }
