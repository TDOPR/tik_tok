...
(
    ...
    `nodeLevel`         tinyint(2) UNSIGNED NOT NULL DEFAULT 0 COMMENT '节点等级 0=普通节点 1=社区节点 2=股东节点',
...
(
    ...
    `totalTaskNum`         bigint UNSIGNED NOT NULL DEFAULT 0 COMMENT '发布关注任务的总次数',
    `hasTaskNum`           bigint UNSIGNED NOT NULL DEFAULT 0 COMMENT '关注可用次数',
    `advertTotalTaskNum`   bigint UNSIGNED NOT NULL DEFAULT 0 COMMENT '发布广告任务的总次数',
    `advertHasTaskNum`     bigint UNSIGNED NOT NULL DEFAULT 0 COMMENT '广告可用次数',
    `createTime`           datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `lastmodifiedTime`     datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`) USING BTREE,
...
(
    ...
    `auditId`          int(0) UNSIGNED NOT NULL COMMENT '审核人',
    `createTime`       datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `lastmodifiedTime` datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
...
(
    ...
    `type`       tinyint(2) UNSIGNED NOT NULL COMMENT '流水类型 1-充值  2-提现  3-购买VIP等级 4-购买任务次数包套餐 5=TTT账户转入 6=购买节点',
...
(
    ...
    `type`       tinyint(2) UNSIGNED NOT NULL COMMENT '流水类型 1-代数奖  2-团队奖 3-分红奖 4=持币奖  5=点赞 6=关注 7=评论 8=转出到USD账号 9=注册福利 10=抵扣购买VIP 11=已过期 12=图片 13=视频 14=抵扣购买投放次数 15=推广广告投放奖励 16=购买节点奖励 17=推广购买节点奖励 18=业绩分红奖励',
    `createTime` datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
(
    ...
    `delayBuyNodeTask` tinyint(2) NOT NULL DEFAULT 0 COMMENT '延迟购买节点发放奖励 1=已执行 0=未执行',
...
(
    ...
    `type`             tinyint(2) UNSIGNED NOT NULL COMMENT '任务类型 1=关注 2=评论 3=点赞  4=图片  5=广告',
    `channel`          tinyint(2) UNSIGNED NOT NULL COMMENT '渠道 1=tiktok  2=抖音',
...
(
    ...
    `type`             tinyint(2) UNSIGNED NOT NULL DEFAULT 1 COMMENT '套餐包类型 1=关注任务 2=广告任务',
...
(
    ...
    `amount`           decimal(12, 2) NOT NULL DEFAULT 0.00 COMMENT '发放的总金额 每天发放的金额=总金额/200',
    `createTime`       datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `lastmodifiedTime` datetime(0) NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`) USING BTREE,
