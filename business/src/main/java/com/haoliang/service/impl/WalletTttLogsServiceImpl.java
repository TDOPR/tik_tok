...
public class WalletTttLogsServiceImpl extends ServiceImpl<WalletTttLogsMapper, WalletTttLogs> implements WalletTttLogsService {
    ...
    public JsonResult<WalletLogsDetailVO> getMybillDetails(PageParam<WalletTttLogs, BillDetailsCondition> pageParam) {
        ...
        if (pageParam.getSearchParam().getType() == -1) {
            //查询所有
            pageData = this.baseMapper.pageByAllType(pageData, userId, beginDate, endDate, dynamicTypeList, taskTypeList, TttLogTypeEnum.getOtherTypeList());
        ...
        depositTypeList.addAll(Arrays.asList(TttLogTypeEnum.ENROLL_IN_BENEFITS.getValue(), TttLogTypeEnum.BUY_NODE_LEVEL_INPUT.getValue()));

        BigDecimal deposit = this.baseMapper.sumAmountByAndUserIdAndTypeInAndDateRange(userId, depositTypeList, beginDate, endDate);
        BigDecimal takeOut = this.baseMapper.sumAmountByAndUserIdAndTypeInAndDateRange(userId, Arrays.asList(TttLogTypeEnum.TO_USD.getValue(), TttLogTypeEnum.BUY_VIP.getValue(), TttLogTypeEnum.BUY_TASK_NUM_PACKAGE.getValue(), TttLogTypeEnum.EXPIRED.getValue()), beginDate, endDate);
    ...
    public JsonResult<CommunityRewardDetailVO> communityRewardDetail(PageDTO pageDTO) {
        Integer userId = JwtTokenUtil.getUserIdFromToken(ThreadLocalManager.getToken());
        AppUsers appUsers = appUserService.selectColumnsByUserId(userId, AppUsers::getNodeLevel);
        ...
        if (pageDTO.isInit()) {
            BigDecimal algebra = BigDecimal.ZERO, team = BigDecimal.ZERO, special = BigDecimal.ZERO, has = BigDecimal.ZERO, advertisement = BigDecimal.ZERO, buyNodeLevel = BigDecimal.ZERO, performance = BigDecimal.ZERO;
            ...
            for (WalletTttLogs walletLogs : walletLogsList) {
                ...
                } else if (walletLogs.getType().equals(TttLogTypeEnum.HOLDING_COINS.getValue())) {
                    has = walletLogs.getAmount();
                } else if (walletLogs.getType().equals(TttLogTypeEnum.POPULARIZE_ADVERT.getValue())) {
                    advertisement = walletLogs.getAmount();
                } else if (walletLogs.getType().equals(TttLogTypeEnum.RECOMMEND_BUY_NODE_LEVEL.getValue())) {
                    buyNodeLevel = walletLogs.getAmount();
                } else if (walletLogs.getType().equals(TttLogTypeEnum.PERFORMANCE_BONUS.getValue())) {
                    performance = walletLogs.getAmount();
                }
            }
            return JsonResult.successResult(CommunityRewardDetailVO.builder()
                    ...
                    .advertisement(NumberUtil.toPlainString(advertisement))
                    .buyNodeLevel(NumberUtil.toPlainString(buyNodeLevel))
                    .performance(NumberUtil.toPlainString(performance))
                    .total(NumberUtil.toPlainString(algebra.add(team).add(special).add(has).add(advertisement).add(buyNodeLevel).add(performance)))
                    .nodeLevel(appUsers.getNodeLevel())
                    .build());
        }
        return JsonResult.successResult(CommunityRewardDetailVO.builder()
                .list(walletLogVOList)
    ...
    @Override
    public JsonResult<PageVO> nodeEarningsDetail(TypeDTO pageDTO) {
        Integer userId = JwtTokenUtil.getUserIdFromToken(ThreadLocalManager.getToken());
        LambdaQueryWrapper<WalletTttLogs> lambdaQueryWrapper = new LambdaQueryWrapper<WalletTttLogs>()
                .select(WalletTttLogs::getCreateTime, WalletTttLogs::getAmount, WalletTttLogs::getType)
                .eq(WalletTttLogs::getUserId, userId)
                .in(WalletTttLogs::getType, Arrays.asList(TttLogTypeEnum.BUY_NODE_LEVEL_INPUT.getValue(),TttLogTypeEnum.RECOMMEND_BUY_NODE_LEVEL.getValue()))
                .orderByDesc(WalletTttLogs::getCreateTime);
        ...
        return JsonResult.successResult(new PageVO(page.getTotal(),page.getPages(),walletLogVOList));
    }
    ...
    public JsonResult taskEarningsDetail(TypeDTO pageDTO) {
        ...
        if (pageDTO.isInit()) {
            ...
            //零撸用户的套餐
            VipOrders zeroOrder = vipOrdersMapper.selectOne(new LambdaQueryWrapper<VipOrders>()
                    .select(VipOrders::getAllowance, VipOrders::getFrozenAmount, VipOrders::getTotal, VipOrders::getCreateTime,VipOrders::getValid)
                    .eq(VipOrders::getUserId, userId)
                    .eq(VipOrders::getLevel, VipLevelEnum.ZERO.getLevel())
                    .eq(VipOrders::getTotal, VipLevelEnum.ZERO.getOutOfSaleAmount())
            );
            ...
            BigDecimal total = this.baseMapper.sumTotalAmountByUserIdAndTypeIn(userId, taskTypeList);

            return JsonResult.successResult(TaskEarningsDetailVO.builder()
                    .zero(new ZeroOrderVO(zeroOrder))
                    .list(walletLogVOList)
    ...
    @Override
    public BigDecimal getYesterdaySumConcernTaskEarnings(List<Integer> typeList) {
        return this.baseMapper.getSumTaskEarningsByTypeIn(typeList);
    }
    ...
    @Override
    public List<TeamTaskDTO> getYesterdaySumTaskEarningGroupByUser(List<Integer> typeList) {
        return this.baseMapper.getYesterdaySumTaskEarningGroupByUser(typeList);
    }
    ...
    @Override
    public BigDecimal sumTotalAmountByTypeListAndParentIdHasMe(List<Integer> taskTypeList, Integer userId) {
        return this.baseMapper.sumTotalAmountByTypeListAndParentIdHasMe(taskTypeList, userId);
    }
