...
public class StatServiceImpl implements StatService {
    ...
    private StatUser buildNewData(LocalDate localDate) {
        //查询昨天的用户
        StatUser statUser = statUserMapper.selectOne(
                new LambdaQueryWrapper<StatUser>()
                        .eq(StatUser::getCreateDate, localDate.minusDays(2))
        );
        int yesterdayNum = 0;
        if (statUser != null) {
     ...
     */
    @Override
    public BusinessVO getByAdmin() {
        List<PortraitSelectVO> user = new ArrayList<>();
        long totalUser = appUserMapper.selectCount(new LambdaQueryWrapper<AppUsers>()
                .eq(AppUsers::getEnabled, BooleanEnum.TRUE.intValue())
        );
        long vaildUser = appUserMapper.selectCount(new LambdaQueryWrapper<AppUsers>()
        ...
        Map<Integer, String> userMap = userCountDTOList.stream()
                .collect(Collectors.toMap(AppUserCountDTO::getLevel, AppUserCountDTO::getCount));
