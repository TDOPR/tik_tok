...
public class AppUserServiceImpl extends ServiceImpl<AppUserMapper, AppUsers> implements AppUserService {
    ...
    public JsonResult<MyDetailVO> getMyDetail() {
        ...
        AppUsers appUsers = this.selectColumnsByUserId(userId, AppUsers::getLevel, AppUsers::getMobile, AppUsers::getValid, AppUsers::getVipLevel, AppUsers::getNickName,
                AppUsers::getInviteCode, AppUsers::getHeadImage, AppUsers::getCreateTime, AppUsers::getNodeLevel);
        ...
        if (appUsers.getNodeLevel() > 0) {
            myDetailVO.setNodeName(MessageUtil.get(String.format("node%d.name", appUsers.getNodeLevel()), ThreadLocalManager.getLanguage()));
        }
    ...
    public JsonResult login(AppUserLoginDTO appUserLoginDTO, String localIp) {
        ...
        updateWrapper.lambda()
                .set(AppUsers::getLoginIp,localIp)
    ...
    public JsonResult register(AppUserRegisterDTO appUserRegisterDTO, String localIp) {
        if (GlobalProperties.isProdEnv()) {
            String cacheKey = CacheKeyPrefixConstants.CAPTCHA_CODE + appUserRegisterDTO.getEmail();
        ...
        if (appUsers == null) {
            ...
            json.put("usdNumber", 0);
            json.put("validDay", TiktokConfig.ZERO_LEVEL_FLUSHED_SEARCH_END_DAY);
            json.put("amount", VipLevelEnum.ZERO.getOutOfSaleAmount().intValue());
            return JsonResult.successResult(json);
        } else {
            return JsonResult.failureResult(ReturnMessageEnum.EMAIL_EXISTS);
        }
    ...
    public JsonResult findPassword(FindPasswordDTO findPasswordDTO) {
        if (GlobalProperties.isProdEnv()) {
            String cacheKey = CacheKeyPrefixConstants.CAPTCHA_CODE + findPasswordDTO.getEmail();
            String code = RedisUtil.getCacheObject(cacheKey);
            if (code == null) {
                return JsonResult.failureResult(ReturnMessageEnum.VERIFICATION_CODE_EXPIRE);
            }
    ...
    @Override
    public JsonResult<NewHomeVO> newHome() {
        List<Banner> bannerList = bannerMapper.selectList(
                new LambdaQueryWrapper<Banner>()
                        .eq(Banner::getEnabled, BooleanEnum.TRUE.intValue())
                        .orderByAsc(Banner::getSortIndex)
        ...
        //查询是否有强制通知的公告
        HomeNoticeVO noticeVO = newsMapper.selectForceNotice(language.toLowerCase());
        return JsonResult.successResult(NewHomeVO.builder()
                .notice(noticeVO)
                .platformDesc(String.format("%shtml/index.html?imgName=%s&language=%s", prevUrl, "platformdesc", language))
                .businessPlan(String.format("%spdf/%s/%s.pdf", prevUrl, language, "businessplan"))
                .productDesc(String.format("%shtml/index.html?imgName=%s&language=%s", prevUrl, "productdesc", language))
                .jobModel(String.format("%shtml/index.html?imgName=%s&language=%s", prevUrl, "jobmodel", language))
                .whitePaper(String.format("%spdf/en_US/whitepaper.pdf", prevUrl))
                .bannerList(bannerUrlList)
                .nodeStrategy(String.format("%shtml/node/index_%s.html", prevUrl, language))
                .build()
        );
    }
}
