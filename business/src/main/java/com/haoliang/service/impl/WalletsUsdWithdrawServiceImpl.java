...
public class WalletsUsdWithdrawServiceImpl extends ServiceImpl<WalletsUsdWithdrawMapper, WalletsUsdWithdraw> implements WalletsUsdWithdrawService {
    ...
    public JsonResult check(AuditCheckDTO auditCheckDTO) {
        ...
        wrapper.lambda()
                ...
                .set(WalletsUsdWithdraw::getAuditId,checkUserId)
    ...
    public JsonResult usdtWithdrawal(UsdtWithdrawalDTO userWalletsDTO) {
        ...
        if(userWalletsDTO.getAmount().compareTo(TiktokConfig.WITHDRAW_MIN_AMOUNT)<0){
            return JsonResult.failureResult(ReturnMessageEnum.WITHDRAW_LIMIT_ERROR.setAndToString(TiktokConfig.WITHDRAW_MIN_AMOUNT));
        }
        ...
        if (GlobalProperties.isProdEnv()) {
            AppUsers appUsers = appUserService.selectColumnsByUserId(userId, AppUsers::getEmail);
            String cacheKey = CacheKeyPrefixConstants.CAPTCHA_CODE + appUsers.getEmail();
    ...
    public Long inertUsdtWithdrawal(WalletsUsdWithdraw walletsUsdWithdraw) {
        if (CoinNetworkSourceEnum.TRX.getName().equals(walletsUsdWithdraw.getCoinType())) {
