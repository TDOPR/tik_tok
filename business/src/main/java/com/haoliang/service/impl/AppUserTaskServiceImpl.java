...
import com.haoliang.model.vo.AdvertDetailVO;
...
public class AppUserTaskServiceImpl extends ServiceImpl<AppUserTaskMapper, AppUserTask> implements AppUserTaskService {
    ...
    public JsonResult accessTask(Long id) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public JsonResult advertTask(Long id) {
        Integer userId = JwtTokenUtil.getUserIdFromToken(ThreadLocalManager.getToken());
        AppUsers appUsers = appUserService.selectColumnsByUserId(userId, AppUsers::getVipLevel, AppUsers::getValid);
        if (appUsers.getVipLevel().equals(VipLevelEnum.ZERO.getLevel())) {
            return JsonResult.failureResult(ResponseStatusEnums.INSUFFICIENT_LIMIT_CODE.getCode(), ReturnMessageEnum.INSUFFICIENT_LIMIT);
        }

        Page<VipOrders> vipOrdersPage = vipOrdersService.page(Page.of(1, 1), new LambdaQueryWrapper<VipOrders>()
                .eq(VipOrders::getUserId, userId)
                .eq(VipOrders::getValid, BooleanEnum.TRUE.intValue())
                .orderByDesc(VipOrders::getAllowance));
        VipOrders vipOrders = null;
        for (VipOrders v : vipOrdersPage.getRecords()) {
            vipOrders = v;
        }

        VipLevelEnum vipLevelEnum = VipLevelEnum.getByLevel(appUsers.getVipLevel());
        if (vipOrders == null || vipOrders.getAllowance().compareTo(vipLevelEnum.getAdvertEarnings()) < 0) {
            //可用余额不足
            return JsonResult.failureResult(ResponseStatusEnums.INSUFFICIENT_LIMIT_CODE.getCode(), ReturnMessageEnum.INSUFFICIENT_LIMIT);
        }

        //判断接单是否达到上限
        int count = this.baseMapper.selectTodaySuccessAdvertNumByUserId(userId);

        if (count >= TiktokConfig.ADVERT_LIMIT) {
            return JsonResult.failureResult(ReturnMessageEnum.TASK_NUM_LIMIT);
        }

        //从使用的vip套餐里冻结相应的金额
        vipOrders.setFrozenAmount(vipOrders.getFrozenAmount().add(vipLevelEnum.getAdvertEarnings()));
        vipOrders.setAllowance(vipOrders.getAllowance().subtract(vipLevelEnum.getAdvertEarnings()));
        vipOrdersMapper.updateById(vipOrders);
        ...
        //保存用户的接单信息
        BigDecimal amount = vipLevelEnum.getAdvertEarnings().divide(kLineDataService.getNowExchangeRate(), TiktokConfig.NUMBER_OF_DIGITS, RoundingMode.FLOOR);
        AppUserTask appUserTask = new AppUserTask();
        appUserTask.setVipOrderIds(JSONObject.toJSONString(Arrays.asList(vipOrders.getId())));
        appUserTask.setTaskId(id);
        appUserTask.setAmount(amount);
        appUserTask.setUserId(userId);
        appUserTask.setLevel(vipLevelEnum.getLevel());
        this.save(appUserTask);
        return JsonResult.successResult();
    }
    ...
    @Override
    public JsonResult advertDetail(Integer type, Long id) {
        if (type == 2) {
            AppUserTask appUserTask = this.getById(id);
            id = appUserTask.getTaskId();
        }
        TiktokTask tiktokTask = tiktokTaskMapper.selectById(id);
        List<String> imgs = JSONObject.parseArray(tiktokTask.getOpusId(), String.class);
        ...
        advertDetailVO.setType(tiktokTask.getType());
        advertDetailVO.setDescription(tiktokTask.getUsername());
        advertDetailVO.setChannel(tiktokTask.getChannel());
        advertDetailVO.setImg(pre + StringUtil.replace(imgs.get(0), rootPath, ""));
        if (tiktokTask.getType().equals(TaskTypeEnum.VIDEO_ADVERT.getType())) {
            advertDetailVO.setVideo(pre + StringUtil.replace(imgs.get(1), rootPath, ""));
        }
        return JsonResult.successResult(advertDetailVO);
    }
...
//                return this.checkTask(auditCheckDTO);
    ...
    @Override
    public JsonResult<PageVO<AppUserTaskVO>> myList(PageParam<AppUserTaskVO, AppUserTaskCondition> pageParam, List<Integer> typeList, Integer channel) {
        Page<AppUserTaskVO> page = this.baseMapper.page(pageParam.getPage(), pageParam.getSearchParam().getStatus(), JwtTokenUtil.getUserIdFromToken(ThreadLocalManager.getToken()), typeList, channel);
    ...
    public JsonResult<PageVO<CheckTaskVO>> findCheckTaskList(PageParam<CheckTaskVO, CheckTaskCondition> pageParam) {
        ...
        for (CheckTaskVO checkTaskVO : page.getRecords()) {
            if (checkTaskVO.getType() == TaskTypeEnum.IMG_ADVERT.getType() || checkTaskVO.getType() == TaskTypeEnum.VIDEO_ADVERT.getType()) {
                if (StringUtil.isNotBlank(checkTaskVO.getOpusId())) {
                    array = JSONArray.parseArray(checkTaskVO.getOpusId());
                    checkTaskVO.setOpusId(pre + StringUtil.replace(array.getString(0), GlobalProperties.getRootPath(), ""));
                }else{
                    checkTaskVO.setOpusId(null);
                }
            }
        }
        return JsonResult.successResult(new PageVO<>(page));
    ...
    public JsonResult checkTask(AuditCheckDTO auditCheckDTO) {
        ...
        Integer checkUserId = JwtTokenUtil.getUserIdFromToken(ThreadLocalManager.getToken());
        ...
        if (TaskStatusEnum.SUCCESS.getCode() == auditCheckDTO.getState()) {
            TiktokTask tiktokTask = this.tiktokTaskMapper.selectById(appUserTaskDTO.getTaskId());
            ...
            walletsService.updateTttWallet(appUserTaskDTO.getAmount(), appUserTaskDTO.getUserId(), FlowingActionEnum.INCOME, TttLogTypeEnum.getTypeByTaskType(tiktokTask.getType()), vipOrderLevel == 0);
        }
        UpdateWrapper<AppUserTask> updateWrapper = Wrappers.update();
        updateWrapper.lambda()
                .set(AppUserTask::getStatus, auditCheckDTO.getState())
                .set(AppUserTask::getAuditTime, LocalDateTime.now())
                .set(AppUserTask::getAuditId, checkUserId)
    ...
    public void checkTaskList() {
        ...
        for (AppUserTaskDTO appUserTaskDTO : appUserTaskDTOList) {
            ...
            //TODO
            walletsService.updateTttWallet(appUserTaskDTO.getAmount(), appUserTaskDTO.getUserId(), FlowingActionEnum.INCOME, TttLogTypeEnum.CONCERN, vipOrderLevel == 0);
            updateWrapper.lambda()
    ...
    @Override
    public Integer selectTodaySuccessAdvertNumByUserId(Integer userId) {
        return this.baseMapper.selectTodaySuccessAdvertNumByUserId(userId);
    }
}
