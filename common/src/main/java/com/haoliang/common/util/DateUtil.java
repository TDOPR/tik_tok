...
public class DateUtil extends cn.hutool.core.date.DateUtil {
     ...
     * 获取今天剩余的秒数
     */
    public static long getToDayRemainingNumberOfSeconds() {
        //获取明天时间
        LocalDate tomorrow = LocalDate.now().plusDays(1);
        //获取当前时间
        LocalDateTime now = LocalDateTime.now();
        return ChronoUnit.SECONDS.between(now, LocalDateTime.of(tomorrow, LocalTime.MIN));
    }
...
//        System.out.println(betweenDays(start, LocalDateTime.now()));
        System.out.println(getYesterdayLocalDate());
//        System.out.println("今天剩余秒数:"+getToDayRemainingNumberOfSeconds());
    }
