...
public class RedisUtil {
     ...
     * @param <T>
     */
    public static <T> void setCacheObjectTodayExpireAsync(final String key, final T value) {
        RBatch batch = getClient().createBatch();
        RBucketAsync<T> bucket = batch.getBucket(key);
        bucket.setAsync(value);
        bucket.expireAsync(Duration.ofSeconds(DateUtil.getToDayRemainingNumberOfSeconds()));
        batch.execute();
    }
